...
export default function LineChartComponent({ title, data }: LineChartComponentProps) {
  ...
  const legends: string[] = Array.from(new Set(data.flatMap(result => result.legend)));
  ...
  return (
    <View className="bg-white shadow-md p-4 rounded-xl mb-6 w-full max-w-md mx-auto flex items-center justify-center">
      {data.some(result => result.datasets[0]?.data?.length > 0) ? (
        <LineChart
          data={{
            labels: data.flatMap(result => result.labels),
            datasets: data.flatMap(result => result.datasets).map((dataset, index) => ({
              ...dataset,
              label: undefined,
