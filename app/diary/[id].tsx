...
const ResultAnalysis = () => {
  ...
  const { skins, acnes, facials } = useHome();
        ...
        <View className="flex justify-center items-center p-4">
          ...
          <View>
            <View className="w-full flex flex-row justify-between items-center">
              ...
              <TouchableOpacity onPress={()=>setIsOpened(!isOpened)} className="flex flex-row items-center gap-x-2">
                <Text>See all</Text>
                {isOpened ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
              </TouchableOpacity>
            </View>
            <View className="w-full flex flex-row flex-wrap">
                {skincareData?.slice(0, isOpened ? skincareData.length : 3).map((item, index) => (
                <CardSkincare key={index} name={item.name} image={item.image} />
                ))}
            </View>
          </View>
        </View>
  ...
  <View className="w-full">
    ...
    >
      {items?.map((item, index) => (
        <AvatarText key={index} title={item.name} image={item.image} />
      ))}
    </View>
  </View>
);
